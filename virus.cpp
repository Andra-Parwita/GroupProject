...
// constuctors
virus::virus()
    : health(0), tileTime(0), id(99), dmg(0), isAlive(false), currentRow(99) {
  srand(time(NULL));
  segmentId = 0;
  explosionCheck = false;
}
virus::virus(int row)
    : health(0), tileTime(0), id(99), dmg(0), isAlive(false), currentRow(row) {
  srand(time(NULL));
  segmentId = 0;
  explosionCheck = false;
}
...
void virus::setPosXY(float x, float y) {
  virus::setPosX(x);
  virus::setPosY(y);
}
...
bool virus::move() {
  if (freeze == false) {
    if ((virus::checkAlive() == true) && (health > 0)) {
      if (internalClock.getElapsedTime().asSeconds() >=
          ((tileTime) + ((rand() % 1000) / 1000))) {
        // std::cout << "moved to: " << position_x -85 << "at: " <<
        // internalClock.getElapsedTime().asSeconds()  << std::endl;
